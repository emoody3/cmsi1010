...
# what I have added:
# 1. speedometer and altitude monitor on the screen that updates as the plane moves
# 2. added the ability to do a touch-and-go, where after landing, the user can press the up arrow key without entering the "down" state first
# 3. added a "GAME OVER" text to appear should the player crash the plane
# 4. added some cloud layers to the background for visual interest
# 5. implemented a score system that rewards the player for successfully entering the "touching" state
# 6. added crash debris that appears when the plane crashes
...
monitor_font = pygame.font.SysFont('sans', 20)
game_font = pygame.font.SysFont('sans', 100)
...
def draw_cloud(x, y):
    ...
    pygame.draw.circle(screen, CLOUD_COLOR, (x + 25, y + 10), 25)
    pygame.draw.circle(screen, CLOUD_COLOR, (x + 60, y), 30)
    pygame.draw.circle(screen, CLOUD_COLOR, (x + 35, y - 10), 20)


def draw_cloud_farther(x, y):
    pygame.draw.circle(screen, (240, 240, 240), (x, y), 15)
    pygame.draw.circle(screen, (240, 240, 240), (x + 20, y + 8), 18)
    pygame.draw.circle(screen, (240, 240, 240), (x + 45, y), 22)
    pygame.draw.circle(screen, (240, 240, 240), (x + 25, y - 8), 15)
...
def draw_crash_debris(x, y):
    for dx, dy in [(-10, -5), (5, -7), (12, 3), (-6, 8), (0, 12)]:
        pygame.draw.rect(screen, (255, 180, 0), (x + dx, y + dy, 4, 4))


def draw_dashboard():
    speed_test = monitor_font.render(
        f"speed: {plane.speed:.1f}", True, (0, 0, 0))
    altitude_text = monitor_font.render(
        f"altitude: {GROUND_LEVEL - plane.y:.1f}", True, (0, 0, 0))
    screen.blit(altitude_text, (10, 10))
    screen.blit(speed_test, (10, 35))


def crashed_screen():
    game_over_font = game_font.render(
        "GAME OVER", True, (255, 0, 0))
    screen.blit(game_over_font, (200, 0))
    final_score = monitor_font.render(
        f"Final Score: {plane.score}", True, (0, 0, 0))
    screen.blit(final_score, (450, 150))
...
def draw_scene():
    if plane.state != "stopped":
        ...
        x = -plane.x
        cloud_x = - (plane.x // 2)
        cloud_x_far = - (plane.x // 4)
        while x < WIDTH:
            draw_tree(x, GRASS_TOP)
            x += TREE_SPACING
        while cloud_x_far < WIDTH:
            draw_cloud_farther(cloud_x_far, 60)
            cloud_x_far += CLOUD_SPACING + 100
        while cloud_x < WIDTH:
            draw_cloud(cloud_x, 100)
            cloud_x += CLOUD_SPACING
        plane.draw()
        plane.move()
        draw_dashboard()
        if plane.state == "crashed":
            crashed_screen()
            draw_crash_debris(WIDTH//2, plane.y)
    clock.tick(60)
    pygame.display.flip()
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_DOWN and plane.state == "flying":
                ...
                plane.rotation = 0
                plane.state = "down"
            elif event.key == pygame.K_UP and plane.state == "touching":
                plane.rotation = 0.2
                plane.state = "rising"
